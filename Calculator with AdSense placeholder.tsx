...
import {
  Calculator, TrendingUp, DollarSign, Users, Building, PiggyBank, Home,
  CreditCard, BarChart3, Lock
} from 'lucide-react';
...
const FosterWealthCalculators = () => {
  ...
  const Field = ({
    id, label, value, onChange, type = 'number', step, hint,
    prefix, suffix, labelColor = 'text-blue-700', className = ''
  }: {
    id: string;
    label: string;
    value: string;
    ...
    return (
      <div className={`space-y-1 ${className}`}>
        <label htmlFor={id} className={`block text-sm font-semibold ${labelColor}`}>{label}</label>
  ...
  // =========================
  const AdBannerTop = () => !showAds ? null : (
  ...
  const AdInContent = () => !showAds ? null : (
    <div className="my-4">
        ...
        if (!isPro) {
          return (
            <div className="p-6 border rounded-lg bg-blue-50 text-blue-900">
              ...
              <p className="mb-4">The Employee Cost calculator is available on the Professional plan.</p>
              <button className="bg-blue-600 text-white px-5 py-2 rounded-lg" onClick={() => (window.location.href = '/upgrade')}>
                Upgrade to Unlock
              </button>
            </div>
          );
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="flex gap-4 mb-3">
                  <label className="flex items-center text-gray-800">
                    <input
                      ...
                      onChange={(e) => setEmployeeInputs({ ...employeeInputs, payType: e.target.value })}
                      className="mr-2"
                    />
                    Salary
                  </label>
                ...
                {employeeInputs.payType === 'salary' ? (
                  <Field
                    ...
                    onChange={(e) => setEmployeeInputs({ ...employeeInputs, baseSalary: e.target.value })}
                  />
                ) : (
                  <div className="grid grid-cols-2 gap-3">
                    <Field
                      ...
                      onChange={(e) => setEmployeeInputs({ ...employeeInputs, hourlyRate: e.target.value })}
                    />
                    <Field
                      id="hoursPerWeek"
                      ...
                      onChange={(e) => setEmployeeInputs({ ...employeeInputs, hoursPerWeek: e.target.value })}
                    />
                  </div>
                )}
                ...
                <Field
                  ...
                  onChange={(e) => setEmployeeInputs({ ...employeeInputs, annualOvertime: e.target.value })}
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <div className="space-y-3">
                  <Field
                    ...
                    onChange={(e) => setEmployeeInputs({ ...employeeInputs, healthInsurance: e.target.value })}
                  />
                  <Field
                    id="retirement401k"
                    ...
                    onChange={(e) => setEmployeeInputs({ ...employeeInputs, retirement401k: e.target.value })}
                  />
                </div>
              </div>
            ...
            <div className="bg-gray-900 text-white p-6 rounded-lg" aria-live="polite">
              ...
              <div className="space-y-4">
                <div className="text-center">
                  <div className="text-3xl font-bold text-yellow-400">{formatCurrency(results.employeeCost.totalCost)}</div>
                  <div className="text-gray-300">Total Annual Cost</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold text-green-400">{formatCurrency(results.employeeCost.hourlyRate)}</div>
        ...
        return (
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-900 text-white p-6 rounded-lg" aria-live="polite">
              ...
              <div className="space-y-4">
                <div className="text-center">
                  <div className="text-3xl font-bold text-green-400">{formatPercent(results.roi.roiPercent)}</div>
                  <div className="text-gray-300">Total ROI</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold text-yellow-400">{formatPercent(results.roi.annualizedROI)}</div>
                  <div className="text-gray-300">Annualized ROI</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-blue-400">{formatCurrency(results.roi.profit)}</div>
        ...
        return (
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-900 text-white p-6 rounded-lg" aria-live="polite">
              ...
              <div className="space-y-4">
                <div className="text-center">
                  <div className="text-3xl font-bold text-yellow-400">{Math.round(results.breakEven.breakEvenUnits)}</div>
                  <div className="text-gray-300">Break-Even Units</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold text-green-400">{formatCurrency(results.breakEven.breakEvenRevenue)}</div>
                  <div className="text-gray-300">Break-Even Revenue</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-blue-400">{formatPercent(results.breakEven.contributionMarginPercent)}</div>
        ...
        return (
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-900 text-white p-6 rounded-lg" aria-live="polite">
              ...
              <div className="space-y-4">
                <div className="text-center">
                  <div className="text-3xl font-bold text-yellow-400">{formatCurrency(results.mortgage.monthlyPayment)}</div>
                  <div className="text-gray-300">Monthly Payment</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold text-green-400">{formatCurrency(results.mortgage.totalInterest)}</div>
                  <div className="text-gray-300">Total Interest</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-blue-400">{formatCurrency(results.mortgage.loanAmount)}</div>
        ...
        return (
          <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="space-y-3">
                  <Field
                    ...
                    onChange={(e) => setSavingsInputs({ ...savingsInputs, currentSavings: e.target.value })}
                  />
                  <Field
                    ...
                    onChange={(e) => setSavingsInputs({ ...savingsInputs, monthlyContribution: e.target.value })}
                  />
                  <div className="grid grid-cols-2 gap-3">
                    <Field
                      id="annualReturn"
                      ...
                      onChange={(e) => setSavingsInputs({ ...savingsInputs, annualReturn: e.target.value })}
                    />
                    <Field
                      id="savingsYears"
                      label="Time"
                      ...
                      onChange={(e) => setSavingsInputs({ ...savingsInputs, timeHorizon: e.target.value })}
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="bg-gray-900 text-white p-6 rounded-lg" aria-live="polite">
              ...
              <div className="space-y-4">
                <div className="text-center">
                  <div className="text-3xl font-bold text-yellow-400">{formatCurrency(results.savings.futureValue)}</div>
                  <div className="text-gray-300">Future Value</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold text-green-400">{formatCurrency(results.savings.totalGrowth)}</div>
                  <div className="text-gray-300">Total Growth</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-blue-400">{formatCurrency(results.savings.totalContributions)}</div>
        ...
        return (
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-gray-900 text-white p-6 rounded-lg" aria-live="polite">
              ...
              <div className="space-y-4">
                <div className="text-center">
                  <div className="text-3xl font-bold text-yellow-400">{results.debtPayoff.payoffYears.toFixed(1)} years</div>
                  <div className="text-gray-300">Payoff Time</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold text-green-400">{Math.round(results.debtPayoff.payoffMonths)} months</div>
                  <div className="text-gray-300">Total Months</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-blue-400">{formatCurrency(results.debtPayoff.totalInterestPaid)}</div>
                  <div className="text-gray-300">Total Interest</div>
                </div>
              </div>
            </div>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (firstTier === 'pro' && !isPro) {
                    const freeKey = Object.entries(suite.calculators).find(([, c]: any) => c.tier === 'free')?.[0] || firstKey;
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-md font-medium transition-all ${
                  activeSuite === key ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-800'
                }`}
              >
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all relative ${
                isActive ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:text-gray-900 border'
              }`}
            >
      ...
      <div className="mt-12 text-center">
        <div className="bg-gradient-to-r from-blue-50 to-green-50 p-8 rounded-lg">
          ...
          <p className="text-gray-600 mb-6">
            Get access to our complete calculator suite with advanced features, custom reports, and priority support.
          </p>
