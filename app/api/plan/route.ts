...
export async function GET(req: Request) {
  try {
    ...
    const plan = (url.searchParams.get("plan") || "plus") as PlanKey;
    const cycle = (url.searchParams.get("cycle") || "mo") as Cycle;
    ...
    // 307 to Stripe Checkout when using legacy /api/plan?plan=...
    return NextResponse.redirect(session.url!, { status: 307 });
  } catch (err: any) {
    return NextResponse.json(
      { error: err?.message || "Plan bridge error" },
      { status: 500 }
    );
  }
}
