...
import { DollarSign, Calculator, Users, Plus, Download, X, Calendar } from "lucide-react";
...
const EmployeeCostPro: React.FC<Props> = ({ onUpgrade }) => {
  ...
  const [newExpense, setNewExpense] = useState<{ name: string; amount: string; frequency: Expense["frequency"] }>({
    name: "",
    amount: "",
    frequency: "annual",
  });
  ...
  const addExpense = () => {
    if (newExpense.name && newExpense.amount !== "") {
      setExpenses((prev) => [
        ...prev,
        { id: Date.now(), name: newExpense.name, amount: Number(newExpense.amount), frequency: newExpense.frequency },
      ]);
  ...
  const getAnnualExpenseAmount = (amount: number, frequency: Expense["frequency"]) => {
    switch (frequency) {
      case "daily":
        return amount * 365;
      case "weekly":
  ...
  const calculateEmployeeCost = () => {
    const basePay =
      employeeData.payType === "salary"
        ? employeeData.salary
        : employeeData.hourlyRate * employeeData.hoursPerWeek * employeeData.weeksPerYear;
    ...
    const totalExpenses = expenses.reduce((sum, e) => sum + getAnnualExpenseAmount(e.amount, e.frequency), 0);
    const totalCost = basePay + benefits + payrollTax + unemploymentTax + workersComp + otherBenefits + totalExpenses;

    const totalHours = Math.max(1, employeeData.hoursPerWeek * employeeData.weeksPerYear); // guard
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-blue-100 mt-2">
              Calculate the true cost of hiring—salary, benefits, taxes, and additional expenses.
            </p>
          </div>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-colors ${
              canDownload ? "bg-white/20 hover:bg-white/30 text-white" : "bg-gray-400 cursor-not-allowed text-gray-200"
            }`}
            disabled={!canDownload}
            title={canDownload ? "Download Report" : "Upgrade to download reports"}
          >
          ...
          <div className="xl:col-span-1 space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <select
                ...
                onChange={(e) => setEmployeeData({ ...employeeData, payType: e.target.value as "salary" | "hourly" })}
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEmployeeData({ ...employeeData, hourlyRate: Number(e.target.value) })}
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="25.00"
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEmployeeData({ ...employeeData, hoursPerWeek: Number(e.target.value) })}
                      className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder="40"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEmployeeData({ ...employeeData, weeksPerYear: Number(e.target.value) })}
                      className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      placeholder="52"
                    />
                  </div>
            ...
            <h4 className="text-lg font-semibold text-gray-700 mb-4">Benefits & Taxes</h4>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEmployeeData({ ...employeeData, benefitsPercent: Number(e.target.value) })}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="25"
                />
                <p className="text-xs text-gray-500 mt-1">Health, dental, vision, etc.</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Payroll Tax (%)</label>
                <input
                  type="number"
                  step="0.01"
                  value={employeeData.payrollTaxRate}
                  onChange={(e) => setEmployeeData({ ...employeeData, payrollTaxRate: Number(e.target.value) })}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="7.65"
                />
                <p className="text-xs text-gray-500 mt-1">FICA (Social Security + Medicare)</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Unemployment Tax (%)</label>
                <input
                  type="number"
                  step="0.01"
                  value={employeeData.unemploymentTax}
                  onChange={(e) => setEmployeeData({ ...employeeData, unemploymentTax: Number(e.target.value) })}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="0.6"
                />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Workers' Comp (%)</label>
                <input
                  type="number"
                  step="0.01"
                  value={employeeData.workersComp}
                  onChange={(e) => setEmployeeData({ ...employeeData, workersComp: Number(e.target.value) })}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="0.75"
                />
                <p className="text-xs text-gray-500 mt-1">Workers' compensation insurance</p>
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setEmployeeData({ ...employeeData, otherBenefits: Number(e.target.value) })}
                className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="0"
              />
              <p className="text-xs text-gray-500 mt-1">Training, equipment, bonuses, etc.</p>
            </div>
            ...
            {/* Additional Expenses */}
            <div>
              <h4 className="text-lg font-semibold text-gray-700 mb-4">Additional Expenses</h4>
              ...
              <div className="space-y-2">
                {expenses.map((expense) => (
                  <div key={expense.id} className="flex justify-between items-center p-3 bg-white border rounded-lg">
                    <div>
                      ...
                      <div className="text-sm text-gray-500">
                        ...
                        {expense.frequency !== "annual" && (
                          <span>
                            {" "}
                            → {formatCurrency(getAnnualExpenseAmount(expense.amount, expense.frequency))} annual
                          </span>
                        )}
                      </div>
                    </div>
          ...
          <div className="xl:col-span-2 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  value={timeBreakdown}
                  onChange={(e) => setTimeBreakdown(e.target.value as typeof timeBreakdown)}
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-200">
              <h4 className="text-lg font-semibold text-blue-700 mb-2">Total {breakdown.period} Cost</h4>
              <p className="text-4xl font-bold text-blue-800">{formatCurrency(breakdown.totalCost)}</p>
              <p className="text-sm text-blue-600 mt-2">
                {((employeeResults.totalCost / employeeResults.basePay - 1) * 100).toFixed(1)}% more than base pay
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                ...
                <p className="text-lg font-bold text-green-800">{formatCurrency(employeeResults.dailyCost)}</p>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                <h5 className="font-semibold text-purple-700">Weekly Cost</h5>
                <p className="text-lg font-bold text-purple-800">{formatCurrency(employeeResults.weeklyCost)}</p>
              </div>
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                <h5 className="font-semibold text-blue-700">Bi-Weekly Cost</h5>
                <p className="text-lg font-bold text-blue-800">{formatCurrency(employeeResults.biWeeklyCost)}</p>
              </div>
              <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                <h5 className="font-semibold text-orange-700">Monthly Cost</h5>
                <p className="text-lg font-bold text-orange-800">{formatCurrency(employeeResults.monthlyCost)}</p>
              </div>
              <div className="bg-red-50 p-4 rounded-lg border border-red-200">
                <h5 className="font-semibold text-red-700">Quarterly Cost</h5>
                <p className="text-lg font-bold text-red-800">{formatCurrency(employeeResults.quarterlyCost)}</p>
              </div>
            </div>

            <div className="bg-gray-50 p-6 rounded-lg">
              <h4 className="font-semibold text-gray-700 mb-4">{breakdown.period} Breakdown</h4>
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-gray-200">
                  <span className="text-gray-700">Base Pay:</span>
                  <span className="font-semibold">{formatCurrency(breakdown.basePay)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-gray-200">
                  <span className="text-gray-700">Benefits ({employeeData.benefitsPercent}%):</span>
                  <span className="font-semibold">{formatCurrency(breakdown.benefits)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-gray-200">
                  <span className="text-gray-700">Payroll Tax ({employeeData.payrollTaxRate}%):</span>
                  <span className="font-semibold">{formatCurrency(breakdown.payrollTax)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-gray-200">
                  <span className="text-gray-700">Unemployment Tax ({employeeData.unemploymentTax}%):</span>
                  <span className="font-semibold">{formatCurrency(breakdown.unemploymentTax)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-gray-200">
                  <span className="text-gray-700">Workers' Comp ({employeeData.workersComp}%):</span>
                  <span className="font-semibold">{formatCurrency(breakdown.workersComp)}</span>
                </div>
                {employeeData.otherBenefits > 0 && (
                  <div className="flex justify-between py-2 border-b border-gray-200">
                    <span className="text-gray-700">Other Benefits:</span>
                    <span className="font-semibold">{formatCurrency(breakdown.otherBenefits)}</span>
                  </div>
                )}
                {breakdown.totalExpenses > 0 && (
                  <div className="flex justify-between py-2 border-b border-gray-200">
                    <span className="text-gray-700">Additional Expenses:</span>
                    <span className="font-semibold">{formatCurrency(breakdown.totalExpenses)}</span>
                  </div>
                )}
                <div className="flex justify-between py-3 pt-4 border-top-2 border-gray-300 text-lg font-bold">
                  <span className="text-gray-800">Total {breakdown.period} Cost:</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                <h5 className="font-semibold text-yellow-800 mb-2">💡 Cost Insights</h5>
                <ul className="text-sm text-yellow-700 space-y-1">
                  <li>
                    • True hourly cost: <strong>{formatCurrencyDetailed(employeeResults.hourlyCost)}</strong>
                  </li>
                  <li>
                    • Benefits & taxes add:{" "}
                    <strong>
                      {formatCurrency(
                        employeeResults.totalCost - employeeResults.basePay - employeeResults.totalExpenses
                      )}
                    </strong>
                  </li>
                  <li>
                    • Additional expenses add: <strong>{formatCurrency(employeeResults.totalExpenses)}</strong>
                  </li>
                  <li>
                    • Cost multiplier:{" "}
                    <strong>{(employeeResults.totalCost / employeeResults.basePay).toFixed(2)}x</strong> base pay
                  </li>
                </ul>
              </div>

              <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-200">
                <h5 className="font-semibold text-indigo-800 mb-2">📊 Pay Period Summary</h5>
                <ul className="text-sm text-indigo-700 space-y-1">
                  <li>• Bi-Weekly: <strong>{formatCurrency(employeeResults.biWeeklyCost)}</strong> (26 periods/year)</li>
                  <li>• Monthly: <strong>{formatCurrency(employeeResults.monthlyCost)}</strong> (12 periods/year)</li>
                  <li>• Quarterly: <strong>{formatCurrency(employeeResults.quarterlyCost)}</strong> (4 periods/year)</li>
