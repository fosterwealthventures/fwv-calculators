...
import { TrendingUp, DollarSign, Users, PiggyBank, Home, BarChart3 } from "lucide-react";
...
const EmployeeCostPro = dynamic(() => import("./EmployeeCostPro"), {
  ssr: false,
  loading: () => <div className="p-4 text-sm opacity-70">Loading Employee Cost…</div>,
});

const ExpenseSplitDeluxe = dynamic(() => import("./ExpenseSplitDeluxe"), {
  ssr: false,
  loading: () => <div className="p-4 text-sm opacity-70">Loading Expense Split Deluxe…</div>,
});
...
const calcMeta = {
  calculators: {
    ...
    "break-even": { name: "Break-Even", icon: BarChart3, tier: "free" as const },
    mortgage: { name: "Mortgage", icon: Home, tier: "free" as const },
    interest: { name: "Interest (Simple/Compound)", icon: PiggyBank, tier: "free" as const },
    "freelancer-rate": { name: "Freelancer Rate", icon: DollarSign, tier: "free" as const },
    "tip-split": { name: "Tip & Tab Split", icon: DollarSign, tier: "free" as const },

    // PLUS
    savings: { name: "Savings Growth", icon: PiggyBank, tier: "plus" as const },
    "debt-payoff": { name: "Debt Payoff", icon: DollarSign, tier: "plus" as const },

    // PRO (Pro choice handled by <Gate/>; Premium always allowed)
    "employee-cost": { name: "Employee Cost", icon: Users, tier: "pro" as const },
    "expense-split-deluxe": { name: "Expense Split Deluxe", icon: PiggyBank, tier: "pro" as const },
  },
} as const;
...
const fmtPct = (n: number) =>
  (isFinite(n) ? n : 0).toLocaleString(undefined, { style: "percent", maximumFractionDigits: 2 });

const ChipButton: React.FC<{ active?: boolean; onClick?: () => void; children: React.ReactNode }> = ({
  active,
  onClick,
  children,
}) => (
  <button
    ...
    className={[
      ...
      active ? "border-brand-green ring-2 ring-brand-green/30" : "border-neutral-300 hover:border-brand-green",
    ].join(" ")}
  >
    {children}
  </button>
  ...
  <div className="inline-flex overflow-hidden rounded-lg border border-neutral-300">
    {options.map((o) => (
      <button
        ...
        className={["px-3 py-2 text-sm", value === o.value ? "bg-brand-green text-white" : "bg-white hover:bg-neutral-50"].join(
          " "
        )}
      >
        {o.label}
...
const ExplanationPanel: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
...
const InputsPanel: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
...
const ResultsPanel: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
 ...
 *  Component
 * ---------------------------------------------------------------------------------------------- */
export default function FosterWealthCalculators({ freeOnly = false }: { freeOnly?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (q && q in calcMeta.calculators) {
      const tier = (calcMeta.calculators as any)[q]?.tier as "free" | "plus" | "pro";
  ...
  const freelancer = useMemo(() => {
    ...
    return { hourly: target / billableHours, weeksWorked, billableHours, target };
  }, [frInputs]);
  ...
  const tipSplit = useMemo(() => {
    ...
    const discount =
      tipInputs.discountMode === "percent" ? bill * (discountValue / 100) : Math.min(discountValue, bill);

    const subtotalAfterDiscount = Math.max(bill - discount, 0);
    const taxBase = subtotalAfterDiscount;
    const taxAmt = taxBase * taxPct;

    const tipBase = tipInputs.tipOn === "beforeDiscount" ? bill : subtotalAfterDiscount;
  ...
  const payoff = useMemo(() => {
    ...
    if (i <= 0) {
      ...
      return { months, totalPaid: Math.max(P, 1) * months, interest: Math.max(P, 1) * months - B };
    }
    if (P <= B * i) return { months: Infinity, totalPaid: Infinity, interest: Infinity };
      ...
      <div className="fwv-container pt-2">
        <div className="flex flex-wrap gap-2">
          {visibleCalcs.map(([key, meta]) => {
            ...
            return (
              <ChipButton key={key} active={active} onClick={() => setActiveCalc(key as CalcKey)}>
                <Icon size={16} />
                {meta.name}
                {meta.tier !== "free" && (
      ...
      <main className="fwv-container mt-6 flex-grow space-y-10 pb-12">
          ...
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-soft">
            ...
            <div className="grid gap-4 p-4 md:grid-cols-2">
              <InputsPanel title="Inputs">
                <Input id="roi_initial" label="Initial Investment" value={roiInputs.initialInvestment} onChange={(v) => setRoiInputs((s) => ({ ...s, initialInvestment: v }))} />
                <Input id="roi_final" label="Final Value" value={roiInputs.finalValue} onChange={(v) => setRoiInputs((s) => ({ ...s, finalValue: v }))} />
                <Input id="roi_time" label="Time Horizon (years)" value={roiInputs.timeHorizon} onChange={(v) => setRoiInputs((s) => ({ ...s, timeHorizon: v }))} />
              </InputsPanel>
            ...
            <div className="px-6 pb-5">
              <ExplanationPanel title="How this works">
                <ul className="ml-5 list-disc">
                  <li><b>ROI:</b> (Final − Initial) ÷ Initial</li>
                  <li><b>Annualized:</b> (Final ÷ Initial)<sup>1/years</sup> − 1</li>
                  <li>Annualizing lets you compare across different time horizons fairly.</li>
                </ul>
                <p className="mt-2">
                  Learn more:{" "}
                  <Link className="text-brand-green underline" href="/guide/roi-vs-annualized-roi">
        ...
        {activeCalc === "break-even" && (
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-soft">
            ...
            <div className="grid gap-6 p-6 md:grid-cols-2">
              <InputsPanel title="Inputs">
                <Input id="be_fixed" label="Fixed Costs" value={beInputs.fixedCosts} onChange={(v) => setBeInputs((s) => ({ ...s, fixedCosts: v }))} />
                <Input id="be_var" label="Variable Cost per Unit" value={beInputs.variableCostPerUnit} onChange={(v) => setBeInputs((s) => ({ ...s, variableCostPerUnit: v }))} />
                <Input id="be_price" label="Price per Unit" value={beInputs.pricePerUnit} onChange={(v) => setBeInputs((s) => ({ ...s, pricePerUnit: v }))} />
              </InputsPanel>
              <ResultsPanel title="Results">
                <KV label="Break-Even (units)" value={isFinite(breakEvenUnits) ? breakEvenUnits : "N/A"} />
              </ResultsPanel>
            </div>
            <div className="px-6 pb-6">
              <ExplanationPanel title="How this works">
                <ul className="ml-5 list-disc">
                  ...
                  <li>If price ≤ variable cost, break-even isn't achievable.</li>
                </ul>
                <p className="mt-2">
                  Learn more:{" "}
                  <Link className="text-brand-green underline" href="/guide/break-even-made-simple">
        ...
        {activeCalc === "mortgage" && (
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-soft">
            ...
            <div className="grid gap-6 p-6 md:grid-cols-2">
              <InputsPanel title="Inputs">
                <Input id="mtg_loan" label="Home Price / Loan Amount" value={mtgInputs.loanAmount} onChange={(v) => setMtgInputs((s) => ({ ...s, loanAmount: v }))} />
                <Input id="mtg_down" label="Down Payment" value={mtgInputs.downPayment} onChange={(v) => setMtgInputs((s) => ({ ...s, downPayment: v }))} />
                <Input id="mtg_rate" label="Interest Rate (%)" value={mtgInputs.interestRate} onChange={(v) => setMtgInputs((s) => ({ ...s, interestRate: v }))} />
                <Input id="mtg_term" label="Loan Term (years)" value={mtgInputs.loanTerm} onChange={(v) => setMtgInputs((s) => ({ ...s, loanTerm: v }))} />
              </InputsPanel>
              <ResultsPanel title="Results">
            ...
            <div className="px-6 pb-6">
              <ExplanationPanel title="How this works">
                <ul className="ml-5 list-disc">
                  <li>Monthly payment uses p = rL / (1 − (1+r)<sup>−n</sup>), where r=rate/12, n=months.</li>
                  <li>Higher rates/longer terms increase total interest paid.</li>
                  <li>Extra principal payments reduce total interest.</li>
                </ul>
                <p className="mt-2">
                  Learn more:{" "}
                  <Link className="text-brand-green underline" href="/guide/mortgage-payment-breakdown">
                    Mortgage Payment Breakdown—Use Our Calculator to See What Impacts Your Monthly Cost
                  </Link>
                </p>
              </ExplanationPanel>
            </div>
        ...
        {activeCalc === "tip-split" && (
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-soft">
            ...
            <div className="grid gap-6 p-6 md:grid-cols-2">
              <InputsPanel title="Inputs">
                <div className="grid gap-4">
                  <Input id="tip_bill" label="Bill Amount ($)" value={tipInputs.bill} onChange={(v) => setTipInputs((s) => ({ ...s, bill: v }))} />
                  <Input id="tip_people" label="Number of People" value={tipInputs.people} onChange={(v) => setTipInputs((s) => ({ ...s, people: v }))} />

                  <div className="flex items-center gap-3">
                    <ToggleGroup
                      value={tipInputs.discountMode}
                      onChange={(v) => setTipInputs((s) => ({ ...s, discountMode: v as "percent" | "amount" }))}
                      options={[
                    ...
                    <Input
                      ...
                      onChange={(v) => setTipInputs((s) => ({ ...s, discountValue: v }))}
                      type="number"
                      placeholder={tipInputs.discountMode === "percent" ? "10 = 10%" : "10 = $10"}
                    />
                  </div>

                  <div className="grid gap-4 sm:grid-cols-2">
                    <Input id="tip_tax" label="Tax (%)" value={tipInputs.taxPct} onChange={(v) => setTipInputs((s) => ({ ...s, taxPct: v }))} />
                    <Input id="tip_tip" label="Tip (%)" value={tipInputs.tipPct} onChange={(v) => setTipInputs((s) => ({ ...s, tipPct: v }))} />
                  </div>

                  <div className="flex items-center gap-3">
                    <span className="text-sm font-medium text-gray-700">Tip On</span>
                    <ToggleGroup
                      value={tipInputs.tipOn}
                      onChange={(v) => setTipInputs((s) => ({ ...s, tipOn: v as "afterDiscount" | "beforeDiscount" }))}
                      options={[
              ...
              <ResultsPanel title="Results">
                <div className="space-y-3">
                  ...
                  <KV label="Subtotal (after discount)" value={fmtUSD(tipSplit.subtotalAfterDiscount)} />
                  <KV label="Tax" value={fmtUSD(tipSplit.taxAmt)} />
                  <KV label="Tip" value={fmtUSD(tipSplit.tipAmt)} />
                  <KV label="Total (with tax & tip)" value={fmtUSD(tipSplit.total)} />
            ...
            <div className="px-6 pb-6">
              <ExplanationPanel title="How this works">
                <ul className="ml-5 list-disc">
                  <li><b>Discount:</b> Choose % or $ off the bill. We cap the discount at the bill amount.</li>
                  <li><b>Tax:</b> Typically applied after the discount; toggle if your locale differs.</li>
                  <li><b>Tip:</b> Common practice is to tip on the discounted, pre-tax subtotal; toggle if needed.</li>
                  <li>Set people to split the final total fairly.</li>
                </ul>
                <p className="mt-2">
                  Learn more:{" "}
                  <Link className="text-brand-green underline" href="/guide/restaurant-tip-tab-split">
          ...
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-soft">
            ...
            <div className="p-6">
              <div className="mb-4 flex gap-2">
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-sm transition ${
                    interestMode === "compound" ? "bg-brand-green text-white" : "border border-neutral-300 hover:bg-neutral-100"
                  }`}
                >
                  Compound
                </button>
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-sm transition ${
                    interestMode === "simple" ? "bg-brand-green text-white" : "border border-neutral-300 hover:bg-neutral-100"
                  }`}
                >
                  Simple
                </button>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <InputsPanel title="Inputs">
                  <Input id="int_principal" label="Principal ($)" value={intInputs.principal} onChange={(v) => setIntInputs((s) => ({ ...s, principal: v }))} />
                  <Input id="int_rate" label="Annual Rate (%)" value={intInputs.annualRatePct} onChange={(v) => setIntInputs((s) => ({ ...s, annualRatePct: v }))} />
                  <Input id="int_years" label="Years" value={intInputs.years} onChange={(v) => setIntInputs((s) => ({ ...s, years: v }))} />
                  {interestMode === "compound" && (
                    <Input id="int_n" label="Compounds / Year" value={intInputs.compoundsPerYear} onChange={(v) => setIntInputs((s) => ({ ...s, compoundsPerYear: v }))} />
                  )}
                </InputsPanel>
                <ResultsPanel title="Results">
            ...
            <div className="px-6 pb-6">
              <ExplanationPanel title="How this works">
                <ul className="ml-5 list-disc">
                  <li><b>Simple:</b> A = P(1 + rt)</li>
                  <li><b>Compound:</b> A = P(1 + r/n)<sup>nt</sup></li>
        ...
        {activeCalc === "freelancer-rate" && (
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-soft">
            ...
            <div className="grid gap-6 p-6 md:grid-cols-2">
              <InputsPanel title="Inputs">
                <Input id="fr_income" label="Desired Annual Income ($)" value={frInputs.annualIncome} onChange={(v) => setFrInputs((s) => ({ ...s, annualIncome: v }))} />
                <Input id="fr_weeks" label="Weeks Off" value={frInputs.weeksOff} onChange={(v) => setFrInputs((s) => ({ ...s, weeksOff: v }))} />
                <Input id="fr_hours" label="Billable Hours / Week" value={frInputs.hoursPerWeek} onChange={(v) => setFrInputs((s) => ({ ...s, hoursPerWeek: v }))} />
                <Input id="fr_oh" label="Overhead (%)" value={frInputs.overheadPct} onChange={(v) => setFrInputs((s) => ({ ...s, overheadPct: v }))} />
              </InputsPanel>
              <ResultsPanel title="Results">
                <KV label="Weeks Worked" value={frInputs.weeksOff ? `${freelancer.weeksWorked}` : freelancer.weeksWorked} />
                <KV label="Billable Hours" value={freelancer.billableHours.toFixed(0)} />
                <KV label="Target Gross" value={fmtUSD(freelancer.target)} />
                <KV label="Required Hourly Rate" value={fmtUSD(freelancer.hourly)} />
              </ResultsPanel>
            </div>
            <div className="px-6 pb-6">
              <ExplanationPanel title="How this works">
                <ul className="ml-5 list-disc">
                  ...
                  <li>Account for vacations, holidays, and non-billable time.</li>
                </ul>
                <p className="mt-2">
                  Learn more:{" "}
                  <Link className="text-brand-green underline" href="/guide/set-your-freelancer-rate-right">
        ...
        {!freeOnly && activeCalc === "savings" && (
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-soft relative">
            ...
            <Gate calc="savings">
              <div className="grid gap-6 p-6 md:grid-cols-2">
                <InputsPanel title="Inputs">
                  <Input id="sav_start" label="Starting Balance ($)" value={sav.start} onChange={(v) => setSav((s) => ({ ...s, start: v }))} />
                  <Input id="sav_monthly" label="Monthly Contribution ($)" value={sav.monthly} onChange={(v) => setSav((s) => ({ ...s, monthly: v }))} />
                  <Input id="sav_rate" label="Annual Return (%)" value={sav.ratePct} onChange={(v) => setSav((s) => ({ ...s, ratePct: v }))} />
                  <Input id="sav_years" label="Years" value={sav.years} onChange={(v) => setSav((s) => ({ ...s, years: v }))} />
                </InputsPanel>
                <ResultsPanel title="Results (monthly compounding)">
                  <KV label="Total Future Value" value={fmtUSD(savings.total)} />
                  <KV label="Total Contributed" value={fmtUSD(savings.contributed)} />
              ...
              <div className="px-6 pb-6">
                <ExplanationPanel title="How this works">
                  <ul className="ml-5 list-disc">
                    <li>
                      FV = P(1+r)<sup>n</sup> + PMT × [((1+r)<sup>n</sup> − 1) / r], with monthly r and n.
                    </li>
                    <li>Change years/contributions to see compounding effects.</li>
                  </ul>
                </ExplanationPanel>
              </div>
            </Gate>
        ...
        {!freeOnly && activeCalc === "debt-payoff" && (
          <section className="rounded-2xl border border-neutral-200 bg-white shadow-soft relative">
            ...
            <Gate calc="debt-payoff">
              <div className="grid gap-6 p-6 md:grid-cols-2">
                <InputsPanel title="Inputs">
                  <Input id="debt_bal" label="Balance ($)" value={debt.balance} onChange={(v) => setDebt((s) => ({ ...s, balance: v }))} />
                  <Input id="debt_apr" label="APR (%)" value={debt.aprPct} onChange={(v) => setDebt((s) => ({ ...s, aprPct: v }))} />
                  <Input id="debt_pay" label="Monthly Payment ($)" value={debt.monthlyPay} onChange={(v) => setDebt((s) => ({ ...s, monthlyPay: v }))} />
                </InputsPanel>
                <ResultsPanel title="Results (fixed payment)">
                  <KV label="Months to Payoff" value={payoff.months === Infinity ? "Payment too small" : payoff.months.toString()} />
              ...
              <div className="px-6 pb-6">
                <ExplanationPanel title="How this works">
                  <ul className="ml-5 list-disc">
                    ...
                    <li>If payment ≤ monthly interest, balance won't fall — increase payment.</li>
                  </ul>
                </ExplanationPanel>
              </div>
            </Gate>
